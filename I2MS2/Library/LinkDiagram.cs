...
//                find_plug_side = (apl.front_asset_id ?? 0) == 0 ? "R" : "F";
        ...
        // asset_id는 지능형 패치만 인수로 받아온다.            
        // 이 함수는 배선뷰(링크다이어그램 편집화면) 에서만 호출된다.
        internal void update_asset_row_status_ic(int asset_id, int start_port_no, int port_no, List<List<WorkCell>> cell_list3, ePortStatus status)
        {
            int y = port_no - start_port_no;
            int x = g.CENTER_COL;
            int idx = y * g.MAX_COL + x;
                ...
                if (cell_list2 != null)
                {
                    ...
                    if (cell != null)
                    {
                        ...
                        if (cell.col_no == g.CENTER_COL) // 중심 셀 변경 처리 
                        {
                            ...
                            update_asset_row_status_right_sw(cable, right, cell_list2, status);
        ...
        // 12번셀의 IPP 관련한 좌측케이블 색상과 11번 패치코드와 관련된 우측케이블 색상 설정을 ...
        private void update_asset_row_status_right_sw(WorkCell cable, WorkCell right, List<WorkCell> cell_list2, ePortStatus status)
        {
            ePortStatus plug_status = status;
            right.left_plug_status = plug_status;
            cable.right_plug_status = plug_status;
            // 우측 IPP의 좌측 케이블 컬러와 중앙에 위치한 패치 케이블의 우측 부분의 컬러를 결정한다.
            if ((plug_status == ePortStatus.Plugged) || (plug_status == ePortStatus.Linked))
        ...
        private void delete_row_when_unplugged(WorkCell left, WorkCell cable, WorkCell right, ePortStatus port_status, List<WorkCell> cell_list2)
        {
            ...
            if ((port_status == ePortStatus.Unplugged) && CatalogType.is_ipp_ic(left.catalog_id))
            {
                del_connected_asset_ic(left.asset_id, left.port_no, ePortStatus.Unplugged, cell_list2);
            }
        }
        ...
        // 호출된 자산의 우측(+2)셀부터 끝까지 삭제...
        public void del_connected_asset_ic(int ipp_asset_id, int port_no, ePortStatus status, List<WorkCell> cell_list2)
        {
            var cell2 = cell_list2.Find(p => (p.asset_id == ipp_asset_id) && (p.port_no == port_no) && (p.col_no == g.CENTER_COL));
            if (cell2 != null)
            {
