...
namespace I2MS2.Library
{
    ...
    public class Stat
    {
        ...
        // 패치 패널 수
        public static int get_tot_pp_ports_by_asset_id(List<int> t2)
        {
        ...
        // 6. 통계 추가 
        // --------------------------------------------------------------------
        public static int get_used_ipp_ports_linked_by_asset_id(List<int> t2)
        {
            int sum = 0;
            var list = from aa in g.asset_list.Where(p => CatalogType.is_ipp(p.catalog_id))
                        join bb in g.catalog_list on aa.catalog_id equals bb.catalog_id
        ...
        // 사용된 지능형 패치 패널 포트 수
        public static int get_tot_ipp_ports_by_asset_id(List<int> t2)
        {
            int sum = 0;
            var list = from aa in g.asset_list.Where(p => CatalogType.is_ipp(p.catalog_id))
                        join bb in g.catalog_list on aa.catalog_id equals bb.catalog_id
        ...
        // 스위치의 전체 포트 수
        public static int get_tot_sw_ports_by_asset_id(List<int> t2)
        {
            int sum = 0;
        ...
        // 사용중인 스위치 포트수 
        public static int get_used_sw_ports_by_asset_id(List<int> t2)
        {
            int sum = 0;
        ...
        // rack 의 전체 공간 수
        public static int get_tot_rack_by_asset_id(List<int> t2)
        {
            int sum = 0;
            var list = from aa in g.rack_list.Where(p => p.rack_catalog_id > 0)
                       ...
                       join cc in g.location_list on aa.rack_id equals cc.rack_id
                       where t2.Contains(cc.location_id)
        ...
        // 사용중인 rack 공간수 
        public static int get_used_rack_by_asset_id(List<int> t2)
        {
            int sum = 0;
            var list = from aa in g.rack_list.Where(p => p.rack_catalog_id > 0)
                       join bb in g.catalog_list on aa.rack_catalog_id equals bb.catalog_id
