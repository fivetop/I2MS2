...
namespace I2MS2
{
    ...
    public partial class App : Application
    {
        ...
        // romee 2015.11.30 서버의 네트웍이나 데이터베이스 연결 이상시 에러 처리용 
        // 시스템 전역 에러 핸들러
        // 별도의 프로세스 필요 -> 에러 마킹후 -> 에러가 사라지면 해당 모듈 실행 처리 필요 
        private static void UnhandledExceptionTrapper(object sender, UnhandledExceptionEventArgs e)
        {
            Console.WriteLine("System Error : Network or Database.. Plz Check I2MS DashBoard System..\n" + e.ExceptionObject.ToString());
...
//            Login win = new Login();
            ...
            string user_id = Reg.get_saved_user_id();
            string enc_password = Reg.get_saved_password();
            //string password = DESEncrypt.Decrypt(enc_password);
            string server = Reg.get_saved_server_domain();
            ...
            var u = g.user_list.Find(p => p.login_id == user_id);
            ...
            if (ret)
            {
                ...
                var mainWindow = new MainWindow3();
        ...
        private async Task<Type> getData()
        {
            ...
            // 2016-12-28 solution
            g.eb_config_list = (List<eb_config>)await g.webapi.getList("eb_config", typeof(List<eb_config>));
            g.eb_target_list = (List<eb_target>)await g.webapi.getList("eb_target", typeof(List<eb_target>));
            _progress.setPercent(++percentage);
