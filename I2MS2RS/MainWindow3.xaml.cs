...
namespace I2MS2
{
    ...
    public partial class MainWindow3 : MetroWindow   
    {
        ...
        {
            SiteNameChange window = new SiteNameChange();
            window.Owner = this;
            window.ShowDialog();
        }
        ...
        private void _cmdSelect_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            var t = new DispatcherTimer(TimeSpan.FromMilliseconds(200), DispatcherPriority.Normal, Tick2, this.Dispatcher);

        }

        private async void Tick2(object sender, EventArgs e)
        {
            var t1 = (DispatcherTimer)sender;
            t1.Stop();

            await g.DVModel.OnLoadView();
        ...
        private void _cmdViewIPM_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (ast == null)
                return;
            int location_id = ast.asset_id;

            IPMViewCCTV window = new IPMViewCCTV(location_id);
        ...
        {
            EnviromentCorrection window = new EnviromentCorrection();
            window.Owner = this;
            window.ShowDialog();
        }
        ...
        private void _cmdScanAll_ICMenu_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            g.webapi.put("site_environment", 99, _item, typeof(site_environment));
        }
        ...
        {
            var obj = System.Reflection.Assembly.GetExecutingAssembly().GetName().Version;
            string version = string.Format("SimpleWin IEMS v{0}.0.4.{1} Client for Windows", obj.Major, obj.Revision);

            MessageBox.Show(this, "Version : " + version);

            //Window win1 = new Alert();
            //win1.Show();
        ...
        {
            g.DVModel.Location_Id = Etc.get_location_id_by_site_id(g.selected_site_id);
            g._dash1.DataContext = g.DVModel;
            g._dash2.DataContext = g.DVModel;
            g._dash3.DataContext = g.DVModel;
            g._dash5.DataContext = g.DVModel;
        }
...
*/
            ...
            if (event_id == 1090173 && eh.asset_id != 0)
            {
                if (g.winAlert != null)
                {
                    if (g.winAlert.isAlive == true)
                        return true;
                }
                g.winAlert = new Alert();
                g.winAlert.Owner = Application.Current.MainWindow;
                g.winAlert.Show();
            }
