...
namespace I2MS2.Library
{
    ...
    public class LinkDiagram
    {
        ...
        public void init_asset_row(int asset_id, int start_port_no, int port_no, List<WorkCell> cell_list2)
        {
            ...
            get_left_asset(asset_id, port_no, ref find_asset_id, ref find_port_no, ref find_plug_side, ref rel_pos);
            ...
            if (a != null)
            {
                ...
                if (c != null)
                {
                    ...
                    {
                        rel_pos = 0;
                        get_right_asset(asset_id, port_no, ref find_asset_id, ref find_port_no, ref find_plug_side, ref rel_pos);
                    }
                }
...
//            else if (CatalogType.is_cp(catalog_id) || CatalogType.is_fp(catalog_id) || CatalogType.is_mb(catalog_id)) // 아울렛인 경우 romee 2015.11.16
//                find_plug_side = (apl.front_asset_id ?? 0) == 0 ? "R" : "F";  // 아웃렛인경우 반대 처리 필요 
            else
        ...
        public void init_asset_row_status(int asset_id, int start_port_no, int port_no, List<WorkCell> cell_list2, bool user_add)
        {
            ...
            // 좌우에 있는 패치 정보를 가져온다.
            int idx2 = get_near_ipp_idx(cell_list2, node);
            if (idx2 == -1)
            {
                // PP가 하나만 있는 경우
                if (aipl != null)
        ...
        private WorkCell find_ipp_cell(List<WorkCell> cell_list2, int asset_id, int port_no)
        {
            ...
            // 발견한 줄에서 패치를 검색한다.
            cell = cell_list2.Find(p => (p.row_no == row_no) && CatalogType.is_ipp(p.catalog_id));
            return cell;
        }
        ...
        private WorkCell get_left_ipp_cell(List<WorkCell> cell_list2, int asset_id, int port_no)
        {
            ...
            if (!cell.is_left_front)
                return cell;
            int idx2 = get_near_ipp_idx(cell_list2, cell);
            if (idx2 != -1)
            {
        ...
        public void get_left_asset(int asset_id, int port_no, ref int find_asset_id, ref int find_port_no, ref string find_plug_side, ref int rel_pos)
        {
            get_last_asset(asset_id, port_no, true, ref find_asset_id, ref find_port_no, ref find_plug_side, ref rel_pos);
        }

        // 화면에 보여줄 가장 우측에 출력할 자산이 검색됨(좌측에 HUB가 있을 경우 사용)
        // ----------------------------------------------------------------------------------------------------
        public void get_right_asset(int asset_id, int port_no, ref int find_asset_id, ref int find_port_no, ref string find_plug_side, ref int rel_pos)
        {
            get_last_asset(asset_id, port_no, false, ref find_asset_id, ref find_port_no, ref find_plug_side, ref rel_pos);
        }

        // 가장 마지막 자산을 알아온다.
        public void get_last_asset(int asset_id, int port_no, bool is_left, ref int find_asset_id, ref int find_port_no, ref string find_plug_side, ref int rel_pos)
        {
