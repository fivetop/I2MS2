...
namespace I2MS2.Library
{
    ...
    public class SignalRClientClass2
    {
        ...
        private int event_proc = 0;


        public DispatcherTimer Timer = new DispatcherTimer();

        public SignalRClientClass2()
        {
        ...
        {
            if (_signalr_list.Count < 1)
                return;
            SignalRMsg msg;

            msg = _signalr_list[_signalr_list.Count - 1];
            //Console.WriteLine("Loss UI={0}", _signalr_list.Count);
            _signalr_list.Clear();
            var b = process_msg(msg);
        ...
        private async Task<bool> process_msg(SignalRMsg msg)
        {
            ...
            switch (type)
            {
                ...
                case eSignalRMsgType.eEvent:
                    ...
                    if (event_proc == 1)
                    {
                        Console.WriteLine("======================================================================> Event Proc Message={0}", msg.event_hist_id);
                        break;
                    }
                    event_proc = 1;
