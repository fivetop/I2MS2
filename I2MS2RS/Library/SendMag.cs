...
namespace I2MS2.Library
{
    class SendMag
    {
        ...
        public bool Send_sms()
        {
            ...
            HttpWebRequest httpWebRequest = HttpWebRequest.Create(g.sms_server) as HttpWebRequest;
            httpWebRequest.Method = "POST";
            //Data to Post to the Page, itis key value pairs; separated by "&"
            string data1 = string.Format("userid={0}&password={1}", g.sms_id, g.sms_pw);
            string data2 = string.Format("{0}&msgcnt=1&msg1={1}", data1, subject);
            string data3 = string.Format("{0}&receivers={1}&sender=01000000000", data2, tosms);
